...
;;
;; Suppose we want to check if all the element values in tree t1 are positive
;;
...
; the result is SAT: all element values in t1 are positive
(assert (= (PAC_forall t1 cleaf_forall_const cpr_forall_const rec_const) true))
(check-sat)
(exit)